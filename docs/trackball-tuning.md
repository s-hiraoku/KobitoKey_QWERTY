...
&pointer_accel {
    min-factor = <800>;
    max-factor = <2500>;             /* scaler で間引く前の最大加速率 */
...
| Layer | 用途 | threshold | tick | tap-ms | max_threshold | wait-ms |
|---|---|---|---|---|---|---|
| 5 | Spaces/Mission Control | 3 | 100 | 10 | 70 | 600 |
| 6 | ブラウザタブ | 3 | 100 | 10 | 70 | 600 |
| 7 | Launchpad/デスクトップ | 3 | 70 | 10 | 50 | 700 |

3 レイヤーとも二重入力対策は tick 増ではなく tap-ms=10 + max_threshold の構造的対策で行う（後述 2026-07-04）。Layer 7 は初回スワイプの取りこぼしを避けるため tick を下げている。Layer 5/6 は縦方向を `&zip_y_scaler 1 2` で 1/2 に減衰し、横スワイプ中の手ブレによる縦誤発火（特に Layer 6 の Cmd+W = タブ閉じ）を防ぐ。
...
**→ 2026-07-04 撤回**: force-awake の run モード常駐によって、静止中の左センサーが
微小ノイズ (±1 カウント) を報告し続け、スクロール経路で「触っていないのに
縦スクロールが動いて画面がブレる」症状が出た（2026-06-20 の「微小 motion が Layer 4
タイマーを延長し続けた」件と同根）。force-awake を撤去し、代わりに REST モードの
サンプリング間隔を短縮して初回スワイプ対策を両立させた:

```conf
# KobitoKey_left.conf
CONFIG_PMW3610_REST1_SAMPLE_TIME_MS=20   # デフォルト 40
CONFIG_PMW3610_REST2_SAMPLE_TIME_MS=40   # デフォルト 100
CONFIG_PMW3610_REST3_SAMPLE_TIME_MS=100  # デフォルト 500
```

- 静止中はセンサーが REST モードに落ちるため、ノイズ報告自体が止まる
- 長時間アイドル後 (REST3) の初回検知遅れは最大 500ms → 100ms に短縮。
  そもそも「初回無反応」の正体はこの 500ms だった可能性が高い
- 消費電力も force-awake 常駐よりは下がる（デフォルトの REST よりは増える）
- これでもまだ勝手にスクロールする場合はタイピング振動によるボールの実移動が
  原因なので、fork した keybind モジュール側にデッドゾーン処理を足すのが次の手
...
### TAB / DESKTOP への横展開と keybind モジュールの fork (2026-07-04)

Layer 5 の構造的 2 連発対策を Layer 6/7 にも展開した:

- `tab_keybind`: tap-ms 30→10、`max_threshold = <70>` 追加（tick=100 に対し最大寄与 96）。
  下スワイプが Cmd+W = タブ閉じなので、2 連発の実害が最も大きいレイヤー
- `desktop_keybind`: tap-ms 30→10、`max_threshold = <50>` 追加（tick=70 に対し最大寄与 68）
- Layer 6 の入力チェーンに `&zip_y_scaler 1 2` を追加。横スワイプ（タブ送り）中の
  手ブレ 1 パケットが縦バケットに入って Cmd+T / Cmd+W が誤発火するのを防ぐ

また、keybind モジュールの `wait-ms` が `k_sleep()` でシステムワークキューを
600〜700ms ブロックする問題（ZMK の temp-layer タイムアウト等、同じキュー上の
遅延処理がジェスチャー直後だけ遅れる）を修正するため、モジュールを fork した:

- fork: [s-hiraoku/zmk-input-processor-keybind](https://github.com/s-hiraoku/zmk-input-processor-keybind) の `non-blocking-wait` ブランチ
- 修正内容: `k_sleep(wait_ms)` を `wait_until` タイムスタンプ + delayed work の
...
### 操作モード追加: ドラッグロック / 精密モード / 横スクロール (2026-07-04)

Layer 4 (MOUSE) に 3 つのモードキーを追加した。いずれもタップ時は通常の文字が出る
ホールドタップにして、Layer 4 誤発動時にタイピングへ被害が出ないようにしている
（K を素通しにした 2026-05-19 の対策を壊さない）。

| 位置 | キー | 長押し | 実装 |
|---|---|---|---|
| 17 | K | ドラッグロック (MB1 押しっぱなし + Layer 10) | `&dl 0 K` → `drag_on` マクロ |
| 23 | V | 右トラボ精密モード (Layer 11) | `&lt 11 V` |
| 29 | ' | 左トラボ横スクロール (Layer 12) | `&lt 12 APOS` |

- **ドラッグロック**: 解除は Layer 10 上の同じキーをタップ (`drag_off`)。長距離ドラッグ中に
  ボタンとボールを同時に押さえ続ける必要がなくなる。LED はシアン(6)で「MB1 が張り付いている」
  状態を可視化。左右の `to0` 位置では `drag_cancel` が MB1 を解放してから Layer 0 へ戻る
- **精密モード**: 押している間、右トラボの移動量を accel の前で 1/2 にする。加速も弱まるので
  小さい UI（閉じるボタン、Figma のハンドル等）を狙いやすい
- **横スクロール**: 通常時も左トラボの X/Y をそれぞれ水平/垂直スクロールへ流す。
  `'`を押している間はYを殺し、Xだけを水平スクロールへ流す横軸ロックになる
- 23 / 29 は `excluded-positions` に追加済み（押しても Layer 4 を解除しない）

### コンボ取りこぼし対策 (2026-07-04)

英数 (D+F) / かな (J+K) が「同時押ししたつもりでも発動せず df / jk が出る」ことが
あったため、`timeout-ms` を 50 → 80 に延長した。D+F / J+K のロールはローマ字にも
英語にもほぼ出ないため誤発動リスクは低い。まだ取りこぼす場合は 100 まで上げる。
ESC (Q+W) / TAB (A+S) は報告がないため 50 のまま（特に A+S は "asa" 等のロールが
多く、延長すると誤発動リスクがある）。全コンボは Layer 0 のみに限定し、NUMBERや
SYMBOLなどで同じ物理位置を同時押ししても発火しないようにした。

### 通常スクロールの2次元化とドラッグ安全終了 (2026-08-05)

左トラックボールの通常経路で捨てていたX軸を有効にし、Magic Trackpadの2本指移動に
近い2次元スクロールへ変更した。横方向は縦操作時の手ブレを抑えるため `1/12`、縦方向は
従来どおり `1/15` に間引いてから水平/垂直ホイールへ写像する。Layer 12は削除せず、
Y軸を無効化する横軸ロックとして残した。

また、Layer 10の左右 `to0` 位置を `drag_cancel` に置き換えた。これによりMB1を明示的に
解放してからLayer 0へ戻るため、ドラッグロック中に通常レイヤーへ戻ってもクリックが
押下状態のまま残らない。

### 上流への還元と west.yml の SHA 固定 (2026-07-04)

- `k_sleep` 修正を上流に PR: [zettaface/zmk-input-processor-keybind#2](https://github.com/zettaface/zmk-input-processor-keybind/pull/2)。
  マージされたらフォーク参照を上流に戻せる
- `west.yml` のブランチ追従だった外部モジュール (badjeff / hsgw / oleksandrmaslov / 自フォーク) を
  コミット SHA に固定し、上流の変更が勝手に入らないようにした
...
- MOUSE Layerと同じposition 34/38の`&to 0`でSCROLL MENUから即座に抜けられる

標準の`zip_temp_layer`は値が0の入力イベントでもタイマーを再設定するため、スケーラーが
余りを蓄積している間の0イベントでLayer 13が残り続けることがあった。SCROLL MENUには
専用の`zip_fixed_temp_layer`を使用し、変換後のスクロール値が非0になった時だけ発動する。
開始後のイベントでは期限を延長せず、各発動を2.5秒の固定期限で解除する。解除後も実際の
スクロールが続いていれば再発動するが、静止中の0値イベントだけでは再発動しない。

### 追加修正 (2026-08-31) — Layer 4 の Y/H/P が無反応になる問題

右トラックボールを動かした直後、`h` や `p` が入力できないことがあった。原因は
Layer 4 (MOUSE) の Y/P/H (position 5/9/15) が素の `&mo 5/6/7` だったこと。`&mo` は
タップしても何も出力しないため、オートマウスレイヤー滞在中はこれらの位置が
無反応になる。さらに同じ position が `excluded-positions` に入っているので、
押してもLayer 4が解除されず、押し直しても文字が出ないままだった。

左手のキーは除外対象外の`&trans`なので、トラックボール操作後に左手を1打すれば
Layer 4は即解除されて正常に戻る。「トラボを触った直後の最初の1打が右手キー」
という条件でのみ発症するため、症状が間欠的に見えていた。

Layer 13で実績のある`menu_lt`（tap-preferred / 180ms）へ置き換え、タップは文字、
長押し中だけ Layer 5/6/7 へ入る形に統一した。`excluded-positions`から外す案は、
長押し判定より先に一時レイヤーが解除されるのを防ぐという除外の目的を壊すため
採用しない。

あわせて右トラボの滞留時間を8000ms→5000msに短縮した。滞留中はU/I/OがMB1/MB3/MB2、
J/LがMB4/MB5に化けるため、窓が長いほど誤爆の機会が増える。3500msで問題になった
「GESTURE/DESKTOPを押す前にLayer 4が切れて文字がリピートする」症状は、Y/H/Pの
`menu_lt`化によって構造的に解消されている（タップは常に文字なのでリピートしない）。

**効果**:
- 右トラボ操作後の`h`/`p`/`y`が短いタップで通常どおり入力できる
- 長押し＋ボール転がしのジェスチャー操作は従来どおり（180msの長押しが必要になる）
- 滞留5秒でU/I/O等がマウスボタンに化ける窓も短縮
