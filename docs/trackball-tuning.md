...
微小ノイズ (±1 カウント) を報告し続け、y×15 のスクロール経路で「触っていないのに
...
- **ドラッグロック**: 解除は Layer 10 上の同じキーをタップ (`drag_off`)。長距離ドラッグ中に
  ...
  状態を可視化。注意: ドラッグ中に `&to 0` を押すと MB1 が張り付いたままになるが、
  ドラッグキーを 2 回押せば復旧する
- **精密モード**: 押している間、右トラボの移動量を accel の前で 1/2 にする。加速も弱まるので
  小さい UI（閉じるボタン、Figma のハンドル等）を狙いやすい
- **横スクロール**: 押している間、左トラボの y を殺して x をスクロールに流す（デフォルトは
  x を殺した縦専用）。倍率は縦の 15 に対し控えめの 8 から開始
