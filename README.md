...
Combos（Layer 0のみ）: `Q+W` = ESC, `A+S` = TAB, `D+F` = 英数(LANG2), `J+K` = かな(LANG1), `BSPC+SYMBOL` = BT Layer
...
控えめにしている。横だけを正確に動かしたい場合は、Layer 4で`'`を長押しする。
...
ズームキーを押していない時は通常のスクロール動作。キーマップは全てtrans（透過）。Layer 4 経由で発動するため、まず右トラックボールに触れて Layer 4 に入った状態でズームキーを押す。

### Layer 10〜12: DRAG / PRECISION / HSCROLL
...
- 左トラックボール: 通常時は2次元スクロール、Layer 5/6/7でジェスチャー、Layer 9で連続ズーム、Layer 12で横軸ロック。`force-awake`は使わず、RESTサンプリング間隔を短縮して初回入力の遅れを抑える。左側からはオートマウス Layer 4 を発動しない
...
- トラックボール操作感: **Magic Trackpad風**にチューニング（詳細は [docs/trackball-tuning.md](docs/trackball-tuning.md)、右は Phase 3 まで適用済み）
  ...
  - オートマウスレイヤー（zip_temp_layer）: 右トラックボールで Layer 4 を自動発動。滞留時間 3500ms / `require-prior-idle-ms`=500ms。トラボから指を離して3.5秒で Layer 0 に自動復帰し、タイピング中の誤発動を抑制
...
- **2次元スクロール + ジェスチャー3系統 + 連続ズーム** — 通常時の上下左右スクロール、Spaces/MC、タブ操作、デスクトップ/Launchpad、Layer 9の連続ズームを左右のトラックボールで操作
