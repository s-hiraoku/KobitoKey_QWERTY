...
├──────┼──────┼──────┼──────┼──────┤  ├──────┼──────┼──────┼──────┼──────┤
│ to0  │ to0  │ to0  │ to0  │ to0  │  │MO(7) │ MB4  │ to0  │ MB5  │MO(9) │
├──────┼──────┼──────┼──────┼──────┤  ├──────┼──────┼──────┼──────┼──────┤
...
- 左トラックボール: スクロール + ジェスチャー（Layer 5/6/7/9時）
...
- トラックボール操作感: **Magic Trackpad風**にチューニング（詳細は [docs/trackball-tuning.md](docs/trackball-tuning.md)、右は Phase 3 まで適用済み）
  ...
  - ジェスチャー（Layer 5/6/7/9）: 用途の重さに応じて threshold/tick/wait-ms を段階設定（誤爆防止）
    ...
    - Layer 9 ズーム: keybindプロセッサを使わず、LCMD保持+通常スクロールで連続ズームを実現（pointer_accelが効くため右トラックボールと同等の滑らかさ）
...
- **トラックボールジェスチャー4系統** — Spaces/MC、タブ操作、デスクトップ/Launchpad、ズーム（Figma等）を使い分け（moNa 2方式）
- **BT/ボリュームをLayer 8に完全隔離** — BSPC+SYMBOLコンボでのみ発動、誤爆の心配なし
- **Layer 5/6/7/9にスタック防止策** — MO()自己参照で安定動作
