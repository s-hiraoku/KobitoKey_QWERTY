...
### Layer 9: ZOOM (MO(9) + 左トラックボール)

右手でMO(9)キー（Layer 4 の右上から2行目右端）を押しながら、左トラックボールを動かすとアプリのズーム操作を発動。Figma / ブラウザ / VSCode などで使える：
...
MO(9)を押していない時は通常のスクロール動作。キーマップは全てtrans（透過）。Layer 4 経由で発動するため、まずトラックボールに触れて Layer 4 に入った状態で MO(9) を押す。
...
- トラックボール操作感: **Magic Trackpad風**にチューニング（詳細は [docs/trackball-tuning.md](docs/trackball-tuning.md)、右は Phase 3 まで適用済み）
  ...
  - ジェスチャー（Layer 5/6/7/9）: 用途の重さに応じて threshold/tick/wait-ms を段階設定（誤爆防止）
    ...
    - Layer 9 ズーム: threshold=4, tick=80, wait-ms=80（連続発火向け：刻んでズーム）
