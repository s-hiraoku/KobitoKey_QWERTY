...
Combos: `D+F` = 英数(LANG2) と `J+K` = かな(LANG1) は全レイヤーで有効。`Q+W` = ESC, `A+S` = TAB, `BSPC+SYMBOL` = BT Layer は Layer 0 のみ。

### Layer 0: DEFAULT (QWERTY)

```
┌──────┬──────┬──────┬──────────┬──────┐  ┌──────────┬──────────┬──────┬──────┬──────────┐
│  Q   │  W   │  E   │    R     │  T   │  │ Y/GESTURE│    U     │  I   │  O   │  P/TAB   │
├──────┼──────┼──────┼──────────┼──────┤  ├──────────┼──────────┼──────┼──────┼──────────┤
│  A   │  S   │  D   │    F     │  G   │  │H/DESKTOP │    J     │  K   │  L   │    ;     │
├──────┼──────┼──────┼──────────┼──────┤  ├──────────┼──────────┼──────┼──────┼──────────┤
│  Z   │  X   │  C   │    V     │  B   │  │    N     │    M     │  ,   │  .   │    '     │
├──────┼──────┼──────┼──────────┼──────┤  ├──────────┼──────────┼──────┼──────┼──────────┤
...
※ Y / H / P: タップは文字、180ms長押し中だけ GESTURE / DESKTOP / TAB。MOUSEレイヤーが切れても同じ位置を押し続ければ文字リピートせず各レイヤーへ入れる。
...
### Layer 4: MOUSE (right auto trackball)

```
┌────────┬────────┬────────┬────────┬────────┐  ┌────────┬────────┬────────┬────────┬────────┐
│        │        │        │        │        │  │GESTURE │  MB1   │  MB3   │  MB2   │  TAB   │
├────────┼────────┼────────┼────────┼────────┤  ├────────┼────────┼────────┼────────┼────────┤
│        │        │        │        │        │  │DESKTOP │  MB4   │DRAG/K  │  MB5   │  ZOOM  │
├────────┼────────┼────────┼────────┼────────┤  ├────────┼────────┼────────┼────────┼────────┤
│        │        │        │PREC/V  │        │  │        │        │        │        │HSCROLL/│
├────────┼────────┼────────┼────────┼────────┤  ├────────┼────────┼────────┼────────┼────────┤
│        │        │        │        │  to0   │  │        │        │        │  to0   │        │
...
- `DRAG/K`: K長押しでドラッグロック、タップでK
- `PREC/V`: V長押し中だけ右トラックボールを精密化、タップでV
- `HSCROLL/'`: `'`長押し中だけ左トラックボールを横軸に固定、タップで`'`
- `ZOOM`: 押している間、左トラックボールで連続ズーム

### 左トラックボール: 2次元スクロール

通常時の左トラックボールは、Magic Trackpadの2本指移動に近い上下左右の
2次元スクロールとして動作する。Xを水平ホイール、Yを垂直ホイールへ変換するため、
スプレッドシートの横スクロールやFigmaのキャンバス移動をレイヤー切替なしで行える。
...
横は `1/12`、縦は `1/15` に間引き、通常の縦操作で横ブレが出にくいよう横感度を
控えめにしている。横だけを正確に動かしたい場合は、SCROLL MENUまたはLayer 4で`'`を長押しする。

左トラックボールで実際のスクロールが発生すると、クリックを含まない安全な
Layer 13 `SCROLL MENU` が2.5秒だけ発動する。0値のセンサーイベントでは発動せず、
同じ発動の期限は操作を続けても延長されない。その間、次のキーを180ms長押しすると、右トラックボールで
Layer 4へ入らなくても左トラックボール機能を呼び出せる。短くタップした場合は
元の文字が入力される。

MOUSEレイヤーと同じ左右親指位置（LCTRL / RALT）には`to 0`があり、
2.5秒のタイムアウトを待たずにSCROLL MENUから抜けられる。トラボを止めた後は
0値イベントで再発動しないため、固定期限でLayer 0へ戻る。

| キー | タップ | 長押し中の左トラックボール |
|---|---|---|
| Y | Y | GESTURE（Layer 0 でも同じ） |
| P | P | TAB（Layer 0 でも同じ） |
| H | H | DESKTOP（Layer 0 でも同じ） |
| ; | ; | ZOOM |
| ' | ' | HSCROLL |

### Layer 5: GESTURE (MO(5) + 左トラックボール)

Y（Layer 0 / SCROLL MENU）またはMO(5)キーを長押ししながら、左トラックボールを動かすとMacのジェスチャー操作を発動：
...
P（Layer 0 / SCROLL MENU）またはMO(6)キーを長押ししながら、左トラックボールを動かすとブラウザのタブ操作を発動：
...
H（Layer 0 / SCROLL MENU）またはMO(7)キーを長押ししながら、左トラックボールを動かすとデスクトップ操作を発動：
...
`;`（SCROLL MENU）またはズームキー（Layer 4 の右上から2行目右端）を長押ししながら、左トラックボールを動かすとアプリの**連続ズーム**を発動。Magic Trackpad のピンチに近い、滑らかな操作感を実現：
...
ズームキーを押していない時は通常のスクロール動作。Layer 4 と SCROLL MENU のどちらからでも発動できる。

### Layer 10〜13: DRAG / PRECISION / HSCROLL / SCROLL MENU

- **Layer 10 DRAG**: MB1を押したままにする。K位置、または左右の`to0`位置で解除する。
  `to0`位置ではMB1を解放してからLayer 0へ戻るため、クリックが張り付かない
- **Layer 11 PRECISION**: Vを長押ししている間、右トラックボールの移動量を1/2にする
- **Layer 12 HSCROLL**: `'`を長押ししている間、左トラックボールのY軸を無効化し、
  `1/8`に間引いたX軸だけを水平スクロールとして送る
- **Layer 13 SCROLL MENU**: 左トラックボール操作後2.5秒だけ発動する安全な入口。
  Y/P/H/;/`'`の長押しでLayer 5/6/7/9/12へ入り、タップ時は元の文字を入力する。
  左右親指位置の`to 0`で即座に終了できる

## 設計方針

- プログラミング（Vim + Mac）向けに最適化
- Layer 0-3: 指で直接入力（文字→機能→数字→記号）
- Layer 4-7, 9-13: トラックボール関連（マウス→ジェスチャー→タブ→デスクトップ→ズーム→ドラッグ→精密→横スクロール→安全な機能入口）
- Layer 8: BT/ボリューム専用（BSPC+SYMBOLコンボで発動）
- BT操作はLayer 8に完全隔離して誤爆防止
- コンボでESC/TABをLayer 0から、英数/かなを全レイヤーから直接入力可能
- 左トラックボール: 通常時は2次元スクロールと同時に安全なLayer 13を2.5秒発動。Y/P/H/;/`'`長押しでLayer 5/6/7/9/12へ入る。クリックを含むLayer 4は発動しない
...
- トラックボール操作感: **Magic Trackpad風**にチューニング（詳細は [docs/trackball-tuning.md](docs/trackball-tuning.md)、右は Phase 3 まで適用済み）
  ...
  - ジェスチャー（Layer 5/6/7）: 用途ごとに threshold/tick/wait-ms を調整。Layer 7 は初回スワイプの反応優先
    - Layer 5 Spaces: threshold=3, tick=100, wait-ms=600
...
- **2次元スクロール + 安全な機能入口** — 通常時の上下左右スクロール後、Y/P/H/;/`'`長押しでジェスチャー3系統、連続ズーム、横軸ロックへ直接移動
...
| Layer 4の左手がほぼ透過    | V長押しの精密モード以外はLayer 0のキーと修飾を流用           | 低     |
