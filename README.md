...
- **レイヤー構成が論理的** — 0-3は指で直接入力（文字→機能→数字→記号）、4-6,8はトラックボール関連（マウス→ジェスチャー→タブ→デスクトップ）、7はBT/ボリューム
