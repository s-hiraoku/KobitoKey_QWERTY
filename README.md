...
- 左トラックボール: スクロール + ジェスチャー（Layer 5/6/7時）+ 連続ズーム（Layer 9時）。初回操作の取りこぼし対策として PMW3610 を ACTIVE 中 `force-awake`。微小入力で Layer 4 が戻らなくなるのを避けるため、左側からはオートマウス Layer 4 を発動しない
...
- トラックボール操作感: **Magic Trackpad風**にチューニング（詳細は [docs/trackball-tuning.md](docs/trackball-tuning.md)、右は Phase 3 まで適用済み）
  ...
  - オートマウスレイヤー（zip_temp_layer）: 右トラックボールで Layer 4 を自動発動。滞留時間 1500ms / `require-prior-idle-ms`=500ms。トラボから指を離して1.5秒で Layer 0 に自動復帰し、タイピング中の誤発動を抑制
