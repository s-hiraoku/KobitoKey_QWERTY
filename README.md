...
### Layer 8: DESKTOP (MO(8) + 左トラックボール)

右手でMO(8)キーを押しながら、左トラックボールを動かすとデスクトップ操作を発動：

| トラックボール方向 | 動作 | キーコード |
|---|---|---|
| → | アプリ切替 | Cmd+Tab |
| ← | アプリ逆切替 | Cmd+Shift+Tab |
| ↑ | Launchpad | F4 |
| ↓ | デスクトップを表示 | F11 |

MO(8)を押していない時は通常のスクロール動作。キーマップは全てtrans（透過）。

### Layer 7: BT (BSPC+ENTER combo)
...
- Layer 7: BT/ボリューム専用（BSPC+ENTERコンボで発動）
- BT操作はLayer 7に完全隔離して誤爆防止
...
- **レイヤー構成が論理的** — 0-3は指で直接入力（文字→機能→数字→記号）、4-6はトラックボール関連（マウス→ジェスチャー→タブ）、7はBT/ボリューム
...
- **BT/ボリュームをLayer 7に完全隔離** — BSPC+ENTERコンボでのみ発動、誤爆の心配なし
- **Layer 5/6にスタック防止策** — MO()自己参照+to0脱出キー
- **LED色でレイヤーが一目瞭然** — 入力系=青、マウス=白、ジェスチャー=ピンク、BT=水色
...
| Layer 4の左手が空き        | BT/ボリューム移動により左手全体が空き。将来の拡張余地        | 低     |
